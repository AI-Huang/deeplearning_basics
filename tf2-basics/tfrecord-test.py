...
TRAINING_SHARDS = 24  # 1024
VALIDATION_SHARDS = 6  # 128

TRAINING_DIRECTORY = 'train'
VALIDATION_DIRECTORY = 'validation'

RAW_DATA_DIR = "D:\\DeepLearningData\\semi-conductor-image-classification-first\\data\\keras"
LOCAL_SCRATCH_DIR = "D:\\DeepLearningData\\semi-conductor-image-classification-first\\data\\scratch"
...
flags.DEFINE_string(
    'local_scratch_dir', LOCAL_SCRATCH_DIR, 'Scratch directory path for temporary files.')
flags.DEFINE_string(
    'raw_data_dir', RAW_DATA_DIR, 'Directory path for raw Imagenet dataset. '
...
def _process_image(filename, coder):
    ...
    """
    # Read the image file.
    # not r, see https://github.com/tensorflow/tensorflow/issues/11312
...
def convert_to_tf_records(raw_data_dir):
    ...
    training_files = tf.gfile.Glob(
        os.path.join(raw_data_dir, TRAINING_DIRECTORY, '*', '*.jpg'))  # not JPEG
    ...
    bad_validation_files = sorted(tf.gfile.Glob(
        os.path.join(raw_data_dir, VALIDATION_DIRECTORY, "bad_1", '*.jpg')))
    ...
    len_bad = len(bad_validation_files)
    bad_validation_synsets = ["bad_1"] * len_bad

    good_validation_files = sorted(tf.gfile.Glob(
        os.path.join(raw_data_dir, VALIDATION_DIRECTORY, "good_0", '*.jpg')))
    len_good = len(good_validation_files)
    good_validation_synsets = ["good_0"] * len_good

    validation_files = bad_validation_files + good_validation_files
    validation_synsets = bad_validation_synsets + good_validation_synsets
    # print(validation_synsets)
    # print("asdha")
    # input()
